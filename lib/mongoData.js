...
module.exports = function (config, db, shareModel) {
	...
	function getMongoContent(options, callback) {
		if (config.debug) {
			console.log('getMongoContent options', options);
		}
		return db.collection(options.collection, function collection(err, col) {
			if (err) {
				if (config.errors) {
					console.error('ERR2 getMongoContent', err);
				}
				return callback(err);
			}
			if (!options.query) {
			...
			if (options.query._id && !(options.query._id instanceof Object)) {
				try {
					...
					if (config.errors) {
						console.error('ERR3 getMongoContent', error);
					}
			...
			return col.findOne(options.query, function foundOne(err, result) {
				if (err) {
					if (config.errors) {
						console.error('ERR4 getMongoContent', err);
					}
					return callback(err);
				}
				if (!result) {
